...
UBYTE _vectorcall IsDateValid(__m128i DMY)
{
	...
	//vectorizing to compare values, these functions require CPU SSE2 support, I won't bother right now to add support for other CPU's, all cpu's 10years back should support SSE2
	const auto comVals = _mm_setr_epi32(daysPrMonth[DMY.m128i_i32[1] - 1], maxMonth, maxYear, 100);//making sure that w = 100 for comparison reasons
	...
	//checking if DMY > comVals with vectorization
